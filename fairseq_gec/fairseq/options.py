...
def parse_args_and_arch(parser, input_args=None, parse_known=False):
    ...
    if parse_known:
        ...
        #print(input_args)
        #print(parser)
        parser.add_argument('--data', nargs='+', default=["./out/data_raw/"],
                           help='data from the interactive.sh')
        args = parser.parse_args(input_args)
        #print(args.data)
        extra = None
    #print(args.memory_efficient_fp16)
    # Post-process args.
...
def get_parser(desc, default_task='translation'):
    ...
    # Added Conflict Resolver @Rajeev
    parser = argparse.ArgumentParser(conflict_handler='resolve', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    # fmt: off
...
def add_interactive_args(parser):
    ...
    group.add_argument('--path', metavar='FILE', default=os.path.join(os.getcwd(), 'out/models/checkpointema9.pt'),
                       help='path(s) to model file(s), colon separated')
    group.add_argument('--beam', default=12, type=int, metavar='N',
                       help='beam size')
    group.add_argument('--nbest', default=1, type=int, metavar='N',
                       help='number of hypotheses to output')
    group.add_argument("--copy-ext-dict", action="store_false",
                       help="Enable copy extended dictionary")
    group.add_argument('--replace-unk', nargs='?', const=True, default=1,
                       help='perform unknown replacement (optionally with alignment dictionary)')
    parser.add_argument('--no-progress-bar', action='store_false', help='disable progress bar')
    group.add_argument('--print-alignment', action='store_false',
                       help='if set, uses attention feedback to compute and print alignment to source tokens')
    group.add_argument('--data', nargs='+', default="./out/data_raw/",
                       help='data from the interactive.sh')
    group.add_argument('--input', default='/home/rohan/rohan/grammar_project/rohan/fairseq-gec/custom_in.txt', type=str, metavar='FILE',
                       help='file to read from; use - for stdin')

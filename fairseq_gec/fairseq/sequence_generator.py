...
class EnsembleModel(torch.nn.Module):
    ...
    def __init__(self, models):
        ...
        self.devices = ['cuda:' + str(next(model.parameters()).get_device()) for model in self.models]
        if all(isinstance(m.decoder, FairseqIncrementalDecoder) for m in models):
            self.incremental_states = {m: {} for m in models}

    @staticmethod
    def dumpToDevice(encoder_input,device):
        dump = {
            'src_tokens': encoder_input['src_tokens'].to(device),
            'src_lengths': encoder_input['src_lengths'].to(device)
        }
        return dump
    ...
    def forward_encoder(self, encoder_input):
        ...
        for i, model in enumerate(self.models):
            encoder_inp = EnsembleModel.dumpToDevice(encoder_input, self.devices[i])
            encoder_outs.append(model.encoder(**encoder_inp))
        return encoder_outs

    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs):
        if len(self.models) == 1:
            for i,(model, encoder_out) in enumerate(zip(self.models,encoder_outs),0):
                tokens = tokens.to(self.devices[i])
                lprobs, attn = self._decode_one(tokens, model, encoder_out, self.incremental_states, log_probs=True)
                return lprobs.to('cpu'), attn.to('cpu') 
        ...
        for i, (model, encoder_out) in enumerate(zip(self.models, encoder_outs),0):
            ...
            log_probs.append(probs.to('cpu'))
            if attn is not None:
                if avg_attn is None:
                    avg_attn = attn.to('cpu')
                else:
                    avg_attn.add_(attn.to('cpu'))
    ...
    def reorder_encoder_out(self, encoder_outs, new_order):
        ...
        out = []
        for i, (model, encoder_out) in enumerate(zip(self.models, encoder_outs),0):
            out.append(model.encoder.reorder_encoder_out(encoder_out, new_order.to(self.devices[i])))
        return out
    ...
    def reorder_incremental_state(self, new_order):
        ...
        for i,model in enumerate(self.models,0):
            model.decoder.reorder_incremental_state(self.incremental_states[model], new_order.to(self.devices[i]))

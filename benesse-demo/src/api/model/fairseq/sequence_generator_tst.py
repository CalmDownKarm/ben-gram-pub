...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        # initialize buffers
        scores = src_tokens.new(bsz * beam_size, max_len + 1).float().fill_(0) #(12,201)
        scores_buf = scores.clone()
        tokens = src_tokens.new(bsz * beam_size, max_len + 2).fill_(self.pad)
        tokens_buf = tokens.clone()
        tokens[:, 0] = bos_token or self.eos #(12,202)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            scores_buf = scores_buf.type_as(lprobs) #torch.Size([12, 201])
            eos_bbsz_idx = buffer('eos_bbsz_idx') #tensor([], device='cuda:0', dtype=torch.int64)
            eos_scores = buffer('eos_scores', type_of=scores) #tensor([], device='cuda:0')
            if step < max_len:
                #set the src_lengths to src_lengths in Search object
                ...
                if prefix_tokens is not None and step < prefix_tokens.size(1):
                    ...
                    #(1,24), (1,24), (1,24)
                    # what is the difference b/w cand_indices and cand_beams?
                    ## I think I know.
            ...
            cand_bbsz_idx = cand_beams.add(bbsz_offsets) #(1,24)

            # finalize hypotheses that end in eos
            # cand_indices is the next (top 24) probable word in the sequence,
            # a hypthesis is said to be complete if it ends in <eos> token.
            # A check if any of the current top predictions is <eos>
            ...
            active_mask = buffer('active_mask') #tensor([], device='cuda:0', dtype=torch.int64)
            torch.add(
            ...
            active_bbsz_idx = active_bbsz_idx.view(-1) #[12]
            active_scores = active_scores.view(-1) #[12]
